(() => {
  document.addEventListener("DOMContentLoaded", () => {
    ...
    function promesaXarxa(i) {
      return new Promise((resolve, reject) => {
        setTimeout(() => {
          let res = simularXarxa(i);
          if (res === 0) {
            reject();
          } else {
            resolve(res);
    ...
    function promesaDiv(i) {
      let pX = promesaXarxa(i)
        .then((res) => Promise.resolve(res))
        .catch(() => {
          console.log("error en la promesa div");
          return promesaDiv(i);
        });
      return pX;
    }
    ...
    for (let i = 0; i < 100; i++) {
      ...
      divResultat.innerHTML = `<h2>R${i}</h2><span class="gif">${getGif()}</span>`;
      contingut.append(divResultat);

      let promesa = promesaDiv(i);
      promesa
        .then((res) => {
          let resultat = document.createElement("p");
          resultat.innerHTML = res;
          divResultat.append(resultat);
          divResultat.querySelector("span.gif").remove();
        })
        .catch(() => {
          console.log("fallo");
        });
...
function getGif() {
  return ` <svg version="1.1" id="L7" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
