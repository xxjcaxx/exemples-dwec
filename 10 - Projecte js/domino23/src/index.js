...
const generatePlayerDiv = (playerTiles,position) => {
   let tiles =  playerTiles.map((tile) => `<span id="tile-${tile}">${getTile(allTiles,tile,position)}</span>`).join('');
...
const generateBoardDiv = (board) => {
    let tiles =  board.map((tile,idx) => `<span id="board-${tile.tile}" data-first = "${idx === 0 ? 'first' : 'no'}" data-last = "${idx === board.length-1 ? 'last' : 'no'}">${tile.tileFigure}</span>`).join('');
    const div = document.createElement('div');
    div.innerHTML = tiles;
    return div;
 }
...
document.addEventListener('DOMContentLoaded',()=>{
    ...
    document.querySelector('#player1').addEventListener('click',(e)=>{
        let tileClicked = e.target.id.split('-')[1];
        if (tileClicked){
    ...
    document.querySelector('#board').addEventListener('click',(e)=>{
        let tileClicked = e.target.id.split('-')[1];
        let locationFirst = e.target.dataset.first;
        let locationLast = e.target.dataset.last;
